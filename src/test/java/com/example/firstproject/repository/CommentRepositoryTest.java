...
class CommentRepositoryTest {
    ...
    void findByArticleId() {
        ...
        /* Case 3: 9번 게시글의 모든 댓글 조회 */
        {
            Long articleId = 9L;
            List<Comment> comments = commentRepository.findByArticleId(articleId);
            Article article = new Article(9L,  "나나나나", "2222");
            List<Comment> expected = Arrays.asList();
    ...
    void findByNickname() {
        ...
        /* Case 3: null의 모든 댓글 조회(특정 닉네임의 입력값이 null일 때) */
        {
            ...
            List<Comment> comments = commentRepository.findByNickname(nickname);
            List<Comment> expected = Arrays.asList();
            assertEquals(expected.toString(), comments.toString(), "null의 모든 댓글은 없음");
        }
        /* Case 4: ""의 모든 댓글 조회(특정 닉네임의 입력값이 없을 때) */
        {
            ...
            List<Comment> comments = commentRepository.findByNickname(nickname);
            List<Comment> expected = Arrays.asList();
            assertEquals(expected.toString(), comments.toString(), "\"\"의 모든 댓글은 없음");
        }
    }
}